...
class pinjamModel {
    ...
    public function tambahPinjam($data) {
        ...
        $data['tgl_kembali'] = date('Y-m-d H:i:s', strtotime($tgl_pinjam . ' +2 days'));
    ...
    public function updateDataPinjam($data) {
        $datas = $this->getPinjamById($data['id']);

        if($data['tgl_kembali'] > $data['tgl_pinjam']){
            $query = "UPDATE pinjaman SET nama_peminjam=:nama_peminjam, jenis_barang=:jenis_barang, no_barang=:no_barang, tgl_pinjam=:tgl_pinjam, tgl_kembali=:tgl_kembali, status='Sudah kembali' WHERE id=:id";
        }else{
    ...
    public function cariPinjam($pinjam){
        ...
        $this->db->bind('nama_peminjam', '%' . $pinjam . '%');
        return $this->db->resultSet();
    }
}
