...
class BukuModel{
    ...
    private function addTwoDaysToToday($tgl_pinjam) {
        $borrowDate = new DateTime($tgl_pinjam);
        return $borrowDate->add(new DateInterval('P2D'))->format('Y-m-d H:i:s');
    }
    ...
    public function tambahBuku($data)
    {
        ...
        $this->db->bind('tgl_kembali',$this->addTwoDaysToToday($data['tgl_pinjam']));
    ...
    {
        $query = "UPDATE tb_pinjam SET nama_peminjam=:nama_peminjam, jenis_barang=:jenis_barang, no_barang=:no_barang, tgl_pinjam=:tgl_pinjam WHERE id=:id";
        $this->db->query($query);
