...
export default {
    ...
    async mounted(){
        // const response = await Get('http://127.0.0.1:9001/api/portfolio');
        const response = await Get('portfolio');
        this.DataPortfolio = response.data.data;
        // const responseCategories = await Get('http://127.0.0.1:9001/api/categories');
    ...
    methods: {
        async filter(id){
            ...
            // Get('http://127.0.0.1:9001/api/portfolio?category_id=' + id);
            this.DataPortfolio = response.data.data;
        }
    }
    // beforeCreate(){
