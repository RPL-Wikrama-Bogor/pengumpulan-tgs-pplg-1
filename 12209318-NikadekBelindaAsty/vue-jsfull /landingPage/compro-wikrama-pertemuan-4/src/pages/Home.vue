...
export default {
  ...
  async created() {
    // api untuk home
    // const response = await Get('http://127.0.0.1:9001/api/home');
    const response = await Get('home');
    this.DataHome = response.data;
    // api untuk layanan
    // const responseLayanan = await Get('http://127.0.0.1:9001/api/services');
    const responseLayanan = await Get('services');
    this.DataLayanan = responseLayanan.data.data;
    // api untuk portfolio
    // const responsePortfolio = await Get('http://127.0.0.1:9001/api/portfolio');
    const responsePortfolio = await Get('portfolio');
    this.DataPortfolio = responsePortfolio.data.data;
    // api untuk blog
    // const responseBlog = await Get('http://127.0.0.1:9001/api/blog');
